...
static inline int world_to_screen_x(float x)
{
    return (int) ((x - WORLD_MIN_X) * gfx_mode_info.x_resolution
                  / (WORLD_MAX_X - WORLD_MIN_X));
}

static inline int world_to_screen_dx(float dx)
{
    return (int) (dx * gfx_mode_info.x_resolution
                  / (WORLD_MAX_X - WORLD_MIN_X));
}

static inline int world_to_screen_y(float y)
{
    return (int) ((y - WORLD_MIN_Y) * gfx_mode_info.y_resolution
                  / (WORLD_MAX_Y - WORLD_MIN_Y));
}

static inline int world_to_screen_dy(float dy)
{
    return (int) (dy * gfx_mode_info.y_resolution
                  / (WORLD_MAX_Y - WORLD_MIN_Y));
}
